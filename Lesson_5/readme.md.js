...
// 2 task
//
// Please create function that will return object with the properties { name, last_name, age, gender }, from the string passed to it, knowing that string will always be like this: "Last Name: last_name; Name: name; Age: age; Gender: gender" where last_name, name, age, gender will be a string values, so the following code will work like this:
//
...
// 3 task
//
// Create a function createPerson(data) that will create Persons inside array PersonsList, if person passed to function is already inside the PersonsList please add the properties that are ont there inside the needed object by index in Array, eg:
//
// var PersonsList = [];
//
...
// 4 task
//
// Create a function timeToRetire that check forEach person in PersonsList if his or her age is more than 50 where person - is object with person data, callback is function that will make some actions with the result of checking:
//
